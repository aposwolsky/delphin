  ...
  *)
  ...
  local 
    ...
    *)
    ...
    fun etaContract (Root (BVar (B), S), s, n) =
        (case (whnfBVar(B, s))
	   of (Fixed k, s') => 
	         (case bvarSub (k, s')
		    of Idx (k') => if k' > n
		  ...
		  (* ABP:  
		   * I assume we are exploiting the condition that
		   * If it was "Exp U" it won't be eta contractable
		   * because it would have been saved as Idx then.
		   *)
		     | _ => raise Eta)
           | (BVarVar _, _) => raise Eta
                      (* BVarVar should always be eta-contracted
		      * but we cannot return a specific index
		      * if it is not resolved...
		      * ABP:  Should we maybe return a variable?
		      *)
       )
    ...
    *)
    ...
    fun findIndex(k, Shift n) =
         (* G',An...A1 |- Shift n : G' *)
         if ((k - n) >= 1) then SOME (k-n)
         else NONE
      | findIndex(k, Dot (Idx n, t)) =
	   if (n=k) then SOME(1)
	   else addOne(findIndex(k, t))
      | findIndex(k, Dot(Exp U, t)) =
	   (* This may be unnecessary... it should be an Idx *)
	   let 
	     val nOpt = SOME(etaContract (U, id, 0))
	            handle Eta => NONE
	   in
	     case nOpt
	       of (SOME n') => if (n'=k) then SOME 1
		               else addOne(findIndex(k, t))
	        | _ => addOne(findIndex(k, t))
	   end 	          
      | findIndex(k, Dot(_, t)) = addOne(findIndex(k,t))
    ...
    *)
    and whnfRoot ((BVar (B), S), s)   =
        (case whnfBVar (B, s)
	   of (Fixed k, s') => 
	        (case bvarSub (k, s')
		   of Idx (k') => (Root (BVar (Fixed k'), SClo (S, s)), id)
	...
	(case blockSub (B, s)
	 ...
	 (* no longer satisfied Wed Nov 27 09:49:58 2002 -fp *)
      ...
      | whnfRoot ((FVar (name, isP, V, s'), S), s) =
	 (Root (FVar (name, isP, V, comp (s', s)), SClo (S, s)), id)
...
(*      | whnf (Us as (Root _, Shift (0))) = Us*)
    ...
    *)
    ...
    fun inferSpine ((Nil, _), Vs) = Vs
	  ...
	  inferSpine ((S, s1), whnfExpandDef (V2, dotEta(Exp (EClo (U, s1)), s2)))
	  (* ABP:  Switched this to "dotEta" from "dot" *)
(* OLD
      | inferSpine ((App (U, S), s1), (Pi (_, V2), s2)) =
...
*)
      ...
      | etaExpand' (U, (Pi ((D, _), V), s)) =
          Lam (decSub (D, s), 
	       etaExpand' (Redex (EClo (U, shift), 
				  App (Root (BVar (Fixed 1), Nil), Nil)), whnfExpandDef (V, dot1 s)))
    ...
    and whnfEtaW (UsVs as (_, (Root _, _))) = UsVs
      ...
      | whnfEtaW ((U, s1), Vs2 as (Pi ((D, P), V), s2)) =
		 ...
		 Redex (EClo (U, comp (s1, shift)), 
			App (Root (BVar (Fixed 1), Nil), Nil))), id), Vs2)
    ...
    *)
    ...
    and normalizeExpW (U as Uni (L), s) = U
      ...
      | normalizeExpW (Us as (EVar r, s)) = EClo (EVar r, normalizeSub s)
    ...
    (* ABP:  4/27/07 *)
      | normalizeSub (Dot (Undef, s)) = Dot(Undef, normalizeSub s)
  ...
  in
    ...
    val etaExpand = etaExpand'

    val whnf = whnf
    val whnfBVar = whnfBVar
