...
struct
  ...
  fun expToApx (I.Uni L) =
    ...
    | expToApx (I.Root (I.FVar (name, _, _, _), _)) =
    ...
    | expToApx (X as I.EVar _) = raise Domain (* ADAM : are we normalized first... expToApx (Whnf.normalize(X, I.id)) *)
  ...
  fun apxToClassW (G, Uni L, _ (* Next L *), allowed) =
    ...
    | apxToClassW (G, V as CVar r, L (* Type or Kind *), allowed) =
      ...
      in
        I.Root (I.FVar (name, false, I.Uni (apxToUni L), s), I.Nil)
      end
  ...
  fun apxToExactW (G, U, (I.Pi ((D, _), V), s), allowed) =
    ...
    | apxToExactW (G, U, Vs as (I.Root (I.FVar (name, isP, _, _), _), s), allowed) =
      let
          ...
          of Level 1 => I.newEVarNdec (G, I.EClo Vs)
           | Level 2 =>
             ...
             let
               ...
               val s' = I.Shift (I.ctxLength (G))
             in
               I.Root (I.FVar (name', isP, V', s'), I.Nil)
             end
      end
