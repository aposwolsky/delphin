  ...
  local
    ...
    *)
    ...
    (* eqFVar F Y = B
       where B iff X and Y represent same variable
    *)
    fun eqFVar (I.FVar (n1, _, _, _)) (FV (n2,  _)) = (n1 = n2)
      | eqFVar _ _ = false
    ...
    *)
    ...
    and occursInBVar (k, (I.Fixed k', s), DP) = 
           (case I.bvarSub (k', s)
	      of I.Idx k2 => if (k= k2) then I.Maybe else DP
               | _ => DP
	     )
      | occursInBVar (k, (I.BVarVar r, s) ,DP) = 
	   (* Check if it is possible for r to be assigned to k *)
	   (case (Whnf.findIndex(k, s))
	      of NONE => DP
	       | (SOME _) => I.Maybe
		)

    and occursInHead (k, I.BVar B, DP) = occursInBVar (k, Whnf.whnfBVar (B, I.id), DP)
    ...
    *)
    ...
    fun collectExpW (G, (I.Uni L, s), K) = K
      ...
      | collectExpW (G, (I.Root (F as I.FVar (name, isP, V, s'), S), s), K) =
    ...
    *)
    fun abstractEVar (I.Decl (K', EV (I.EVar(r',_,_,_))), depth, X as I.EVar (r, _, _, _)) =
        if r = r' then I.BVar (I.Fixed (depth+1))
...
(*      | abstractEVar (I.Decl (K', FV (n', _)), depth, X) = 
    ...
    (* abstractFVar (K, depth, F) = C'
       ...
       and  {{K}}, G |- C' : V
    *)
    fun abstractFVar (I.Decl(K', FV (n', _)), depth, F as I.FVar (n, _, _, _)) = 
