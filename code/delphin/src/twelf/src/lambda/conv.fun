  ...
  local
    ...
    *)
	...
	(* G |- D1[s1] = D2[s2] by typing invariant *)
	  ...
	  convExp ((U1, dot1 s1),
		   (Redex (EClo (U2, shift), 
			   App (Root (BVar (Fixed 1), Nil), Nil)), dot1 s2)) 

      | convExpW ((U1,s1), (Lam(D2, U2) ,s2)) =                           
	  convExp ((Redex (EClo (U1, shift), 
			   App (Root (BVar (Fixed 1), Nil), Nil)), dot1 s1),
		   (U2, dot1 s2))
    ...
    *)
    and convSub (Shift(n), Shift(m)) = true (* n = m by invariants *)
	  ...
	  (case (Ft1, Ft2) of
	   ...
	   | (Exp (U1), Idx (n2)) => convExp ((U1, id), (Root (BVar (Fixed n2), Nil), id))
           | (Idx (n1), Exp (U2)) => convExp ((Root (BVar (Fixed n1), Nil), id), (U2, id))
	   | (Undef, Undef) => true
	   | _ => false)
    ...
    *)
    and convDec ((Dec (_, V1), s1), (Dec (_, V2), s2)) =  convExp ((V1, s1), (V2, s2))
      ...
      | convDec ((NDec, s1), (NDec, s2)) = convSub (s1, s2)
