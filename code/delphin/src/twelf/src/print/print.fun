  ...
  *)
  fun subToSpine (depth, s) =
      let fun sTS (I.Shift(k), S) =
		...
		sTS (s, I.App(I.Root(I.BVar(I.Fixed k), I.Nil), S))
	    | sTS (I.Dot(I.Exp(U), s), S) =
		sTS (s, I.App(U, S))

	  (* ABP: Added case for Undef. (just ignore it for printing)
	   * This can happen because of NDecs, namely
	   * "Undef : NDec"
	   * So it is not the case that Undefs are just for
	   * strengthening substitutions.. when/if we remove NDecs
	   * then this case can go away
	   *)
	    | sTS (I.Dot(I.Undef, s), S) =
		sTS (s, S)
