...
sig
  ...
  val pid : string list * string * Paths.region -> term (* parameter id *)
  ...
  val fvar : string * bool * Paths.region -> term
  ...
  val dec : string option * term * (Approx.Exp option) * Paths.region -> dec (* id : tm | _ : tm *)
  val dec0 : string option * Paths.region -> dec (* id | _  (type omitted) *)
  val getStringOption : dec -> string option (* added by ABP *)
  val ndec : string option * Paths.region -> dec
...
sig
  ...
  val jscopeVars : job -> job
  val jpopctx : int * job -> job (* Added by ABP *)
  val jterm : term -> job
  val jclass : term -> job
  val jof : term * term -> job
  val jofApx : term * Approx.Exp -> job
  val jtypeEqualApx : term * Approx.Exp -> job (* Added by ABP .. returns a job of JClass *)
