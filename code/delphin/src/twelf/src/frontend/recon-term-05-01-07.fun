...
struct
  ...
  datatype term =
    ...
    | typevar of string * Paths.region (* ABP:  2/14/04 *) (* THIS CAN NOW BE DELETED.. IT IS NOT USED *)
    | typ of Paths.region
    | arrow of term * term
    | pi of dec * term
    | lam of dec * term
  ...
  and dec =
      ...
      | decRef of (string option * term * Paths.region * ((term  * Apx.Exp) option) * IntSyn.Exp option ) ref (* ABP -- 9/12/04 *)
                         (* THIS (decRef) CAN NOW BE DELETED.. IT IS NO LONGER USED *)
  ...
  fun getStringOption (dec (name, _, _)) = name
    | getStringOption (decRef (ref (name,_,_, _, _))) = name
  ...
  in
    ...
    and inferApxDec (G, dec (name, tm, r)) =
	  ...
	  val _ = case Aopt 
		    ...
		    let
		      ...
		      val _ = match (V1, tm'') handle Unify s => fatalError(termRegion tm, "Delphin:  Term of incompatible type.  Error:  " ^ s) (* ADAM *)
		    in
		      ()
		    end
		   | NONE => ()
	...
	let
	  ...
	  val (tm'', _) = Apx.expToApx (Whnf.normalize(A, IntSyn.id))
          val _ = filterLevel (tm', L, 2,
                               "The term in this position must be an object or a type family")
	  val _ = match (U, tm'') handle Unify s => fatalError(termRegion tm, "Delphin:  Term of incompatible type.  Error: " ^ s) (* ADAM *)
          val _ = runDelayed ()
        in
          jterm (tm')
        end
