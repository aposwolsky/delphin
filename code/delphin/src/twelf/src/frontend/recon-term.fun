...
functor ReconTerm ((*! structure IntxSyn' : INTSYN !*)
...
struct
  ...
  fun raiseType (Null, V) = V
    | raiseType (Decl (G, NDec), V) = raise Domain (* raiseType (G, EClo(V, shift)) *)
    | raiseType (Decl (G, D), V) = raiseType (G, Pi ((D, Maybe), V))

  end (* open IntSyn *)

  local
    val evarApxTable : Apx.Exp StringTree.Table = StringTree.new (0) (* ABP:  What is "evar" used for?  "_" is omitted *)
    val fvarApxTable : Apx.Exp StringTree.Table = StringTree.new (0)

    val fvarCtx : (IntSyn.Dec IntSyn.Ctx) ref = ref IntSyn.Null
    val fvarTable : (int*IntSyn.Exp) StringTree.Table = StringTree.new (0) (* int refers to number of elements in the context *)

  in

    datatype Action 
      = AddEapx of string * Apx.Exp
      | AddFapx of string * Apx.Exp
      | AddF of string * IntSyn.Exp
    ...
    fun saveApxTables () = (StringTree.save evarApxTable, StringTree.save fvarApxTable)
    fun saveExactTables () = (!fvarCtx, StringTree.save fvarTable)

    fun setfvarCtx G = fvarCtx := G

    fun restoreApxTables (evarApxT, fvarApxT) =
      (StringTree.restore (evarApxTable, evarApxT); 
       StringTree.restore (fvarApxTable, fvarApxT))

    fun restoreExactTables (ctx, fvarT) = (fvarCtx := ctx ; StringTree.restore (fvarTable, fvarT))
           ...
           of SOME (X as IntSyn.EVar (_, G, V, _)) => (X, raiseType (G, V))
            | NONE =>
              let
                ...
                val X = IntSyn.newEVar (G'', V'') (* don't need newEVarNDec here *)
              in
           ...
           of SOME (n,V) => (n,V)
            | NONE =>
              let
                ...
                (* val V' = Apx.apxToClass (IntSyn.Null, V, Apx.Type, allowed) *)
		val V' = Apx.apxToClass (!fvarCtx, V, Apx.Type, allowed) 
		val n = IntSyn.ctxLength (!fvarCtx)
              in
                StringTree.insert fvarTable (name, (n, V'));
                (n, V')
              end)
  ...
  datatype term =
    ...
    | fvar of string * bool * Paths.region (* bool=true if it is a parameter *)
    | typ of Paths.region
    | arrow of term * term
    | pi of dec * term
    | lam of dec * term
    ...
    | hasApproxType of term * Apx.Exp (* Added by ABP -- 5/1/07 *) (* NOT USED.. CAN BE REMOVED *)
    ...
    | pid of string list * string * Paths.region
  ...
  and dec =
      dec of string option * term * (Approx.Exp option) * Paths.region (* ABP -- 5/10/07 Allow us to specify an approx type it must be equal to *)
      | ndec of string option * Paths.region (* ABP -- 8/17/04 *)


  fun backarrow (tm1, tm2) = arrow (tm2, tm1)
       
  (* for now *)
  fun dec0 (nameOpt, r) = dec (nameOpt, omitted (r), NONE, r)

  fun getStringOption (dec (name, _, _, _)) = name
    | getStringOption (ndec (name, _)) = name
    
  (* Throw out the last i elements of G *)
  fun popCtx (0, G) = G
    | popCtx (i, IntSyn.Decl(G, _)) = popCtx(i-1, G)
    | popCtx (i, IntSyn.Null) = raise Domain (* broken invariant *)

  datatype job =
      jnothing
    | jand of job * job
    | jwithctx of dec IntSyn.Ctx * job
    | jscopeVars of job (* Added by ABP *)
    | jscopeVars2 of ((Apx.Exp StringTree.Table) * (Apx.Exp StringTree.Table)) * job (* Saves a log of variables in first argument *)
    | jpopctx of int * job (* Pop the last i elements of the current context 
			    * Added:  ABP -- May 02 2007 
			    *)
    | jterm of term
    | jclass of term
    | jof of term * term
    | jofApx of term * Approx.Exp (* M ~:~ A ~:~ Type *)
    | jtypeEqualApx of term * Approx.Exp (* Added ABP -- May 10 2007 *)
  ...
  fun termRegion (internal (U, V, r)) = r
    ...
    | termRegion (hastype (tm1, tm2)) =
        Paths.join (termRegion tm1, termRegion tm2)
    | termRegion (hasApproxType (tm1, _)) =
        (termRegion tm1)
  ...
  in
    ...
    fun findEFVar fc (G, qid, r) =
        (case Names.unqualified qid
           of NONE => fc (G, qid, r)
            | SOME name => (if !queryMode then evar else (fn (s, r) => fvar(s, false (* it is not a parameter *), r))) (name, r))
    ...
    fun inferApx (G, tm as internal (U, V, r)) =
      ...
      | inferApx (G, tm as pid (ids, name, r)) =
        let
          val qid = Names.Qid (ids, name)
        in
          inferApx (G, findPID (G, qid, r))
        end
      ...
      | inferApx (G, tm as fvar (name, isP, r)) =
	...
	in
	  ...
	  handle Unify problem_msg =>
	    let
	      val location_msg = "Delphin: Type mismatch"
	      val r = termRegion tm
	      val (tm'', U'') = checkApx (G, omitted r, A, L, location_msg)
	      (* just in case *)
	      val _ = addDelayed (fn () => (makeGroundUni L ; ()))
	    in
	      (mismatch (tm', tm'', location_msg, problem_msg), U'', A, L)
	    end
	end
    ...
    and inferApxDec (G, dec (name, tm, NONE, r)) =
        let
          val (tm', V1) = checkApx (G, tm, Uni Type, Kind,
                                    "Classifier in declaration must be a type")
          val D = Dec (name, V1)
        in
          (dec (name, tm', NONE, r), D)
        end
      | inferApxDec (G, dec (name, tm, SOME A, r)) =
        let
          val (tm', V1) = checkApx (G, tm, Uni Type, Kind,
                                    "Classifier in declaration must be a type")
	  val (tm', V1) = (match (V1, A) ; (tm', V1))
	    	  handle Unify problem_msg =>
		    let
		      val location_msg = "Delphin: Type mismatch"
		      val r = termRegion tm'
		      val tm2 = omitapx(A, Uni Type, Kind, r) 
		    in
		      (mismatch (tm', tm2, location_msg, problem_msg), A)
		    end

          val D = Dec (name, V1)
        in
          (dec (name, tm', SOME A, r), D)
        end
      | inferApxDec (G, ndec (name, r)) = (ndec (name, r), NDec)
    ...
    fun inferApxJob (G, jnothing) = jnothing
      ...
      | inferApxJob (G, jscopeVars j) = 
	   let
	     val oldTables = saveApxTables ()
	     val job = inferApxJob(G, j)
	     val newTables = saveApxTables()
	     val _ = restoreApxTables oldTables
	   in
	     jscopeVars2 (newTables, job)
	   end


      | inferApxJob (G, jpopctx (i, j)) =
	   jpopctx (i, inferApxJob (popCtx(i, G), j))
      ...
      | inferApxJob (G, jofApx (tm1, V)) =
        let
          val _ = clearDelayed ()
          val L = Apx.Type (* tm1 ~:~ V ~:~ Type *)
          val (tm1', U1) = checkApx (G, tm1, V, L,
                                     "Ascription in declaration did not hold")
          val _ = filterLevel (tm1', L, 2,
                               "The term in this position must be an object or a type family")
          val _ = runDelayed ()
      ...
      | inferApxJob (G, jtypeEqualApx (tm, Vspecified)) =
        let
	  (* tm ~:~ Type ~:~ Kind *)
          val _ = clearDelayed ()
          val (tm', V) = checkApx (G, tm, Uni Type, Kind,
                                   "The term in this position must be a type")

	  val tm' = (match (Vspecified, V) ; tm')
	    handle Unify problem_msg =>
	    let
	      val location_msg = "Delphin: Type mismatch (Equality does not hold)"
	      val r = termRegion tm
	      val tm2 = omitapx(Vspecified, Uni Type, Kind, r) 
	    in
	      (mismatch (tm', tm2, location_msg, problem_msg))
	    end

          val _ = runDelayed ()
        in
          jtypeEqualApx (tm', Vspecified)
        end
  ...
  datatype Job =
    ...
    | JPopCtx of int * Job (* Added by ABP *)
  ...
  fun bvarElim (Fixed n, s0) = (fn (s, S) =>
      (case bvarSub (n, comp(s0,s))
         of Idx (n') => Root (BVar (Fixed n'), S) 
          | Exp (U) => Redex (U, S)))
    | bvarElim _ = raise Domain (* I guess we do need to handle BVarVar in reconstruction *)

  fun fvarElim (name, isP, V, s) =
        (fn (s', S) => Root (FVar (name, isP, V, comp (s, s')), S))
  fun redexElim (U) = (fn (s, S) => Redex (EClo (U, s), S))
  (* headElim (H) = E
     assumes H not Proj _ *)
  fun headElim (BVar B) = bvarElim (Whnf.whnfBVar (B, id))
  ...
  fun etaExpandW (E, (Pi ((D as Dec (_, Va), _), Vr), s)) =
      let
        val U1 = etaExpand (bvarElim ((Fixed 1, id)), (Va, comp (s, shift)))
        val D' = decSub (D, s)
      in
  ...
  fun reportInfer' (G, omitexact (_, _, r), U, V) =
    ...
    | reportInfer' (G, hasApproxType _, U, V) = ()
  ...
  fun reportInfer x = report (fn () => reportInfer' x)
    ...
    fun inferExactN (G, tm as internal (U, V, r)) =
      ...
      | inferExactN (G, tm as fvar (name, isP, r)) = 
        let
          val (n, V) = getFVarType (name, false)
                  handle Apx.Ambiguous =>
                  let
                    val (n, V) = getFVarType (name, true)
                  in
                    delayAmbiguous (G, V, r, "Free variable has ambiguous type");
                    (n, V)
                  end
	   (* val s = Shift (ctxLength (G)) (* necessary? -kw *) *)
           val n' = IntSyn.ctxLength G;
	   val s = if (n' >= n) then Shift (n'-n) else
	                 (* else fvar is not in an extended context...*)
	                 (fatalError(r, "FVar out of scope. (did you pop it away?)") ; id)
        in
          (tm, Elim (fvarElim (name, isP, V, s)), EClo (V, s))
        end
      ...
      | inferExactN (G, hasApproxType (tm, V)) =
        let
	  val L = Apx.Type (* U(tm) ~:~ V ~:~ Type *)
          val V' = Apx.apxToClass (G, V, L, false)
		     ...
		     val r = termRegion tm
                   in
                     delayAmbiguous (G, V', r, 
				     "Delphin: term has ambiguous type") ; 
		     V'
		   end

	  val (tm', B) = checkExact (G, tm, (V', id), "Delphin:  Ascription did not hold\n")
	in
	  (hasApproxType (tm', V), B, V')
	end
	 ...
	 (* ABP:  May, 2007.. This is a BUG..  no?  They both can have the same type! ..
	  * i.e. nat : type  and exp : type 
	  * So what use is it to print out type != type
	  * BUT.. are you purposefully printed out the types of the mismatched terms.. if so.. why?
	  OLD
	    ...
	    *)
	  val U1 = toIntro(B1, (V1, id))
	  val U2 = toIntro(B, (V, id))
          val _ = if !trace then reportMismatch (G, (U1, id), (U2, id), problem_msg) else ()
          val _ = delayMismatch (G, U1, U2, termRegion tm2', location_msg, problem_msg)
        in
    ...
    and inferExactDec (G, dec (name, tm, _, r)) =
        let
      ...
      | inferExactDec (G, ndec (name, r)) = (ndec (name, r), NDec)

    and checkExact1 (G, lam (dec (name, tm1, apxOption, r), tm2), Vhs) =
        let
          ...
          val U2 = toIntro (B2, (V2, id))
        in
          ((lam (dec (name, tm1', apxOption, r), tm2'), Intro (Lam (D, U2)), Pi ((D, Maybe), V2)), ok2)
        end
      ...
      | checkExact1 (G, hasApproxType (tm, V (* = Vhs *)), Vhs) = 
	let
	  val ((tm', B1, V'), ok) = checkExact1(G, tm, Vhs)
	in
	  ((hasApproxType(tm', V), B1, V'), ok)
	end
    ...
    and unifyExact (G, arrow (tm1, tm2), Vhs) =
      ...
      | unifyExact (G, pi (dec (name, tm1, apxOption, r), tm2), Vhs) =
        let
          val (Pi ((Dec (_, Va), _), Vr), s) = Whnf.whnfExpandDef Vhs
          val ((tm1', B1, _ (* Uni Type *)), ok1) = unifyExact (G, tm1, (Va, s))
          val V1 = toIntro (B1, (Uni Type, id))
          ...
          val V2 = toIntro (B2, (L, id))
        in
          ((pi (dec (name, tm1', apxOption, r), tm2'), Intro (Pi ((D, Maybe), V2)), L), ok2)
        end
        (* lam impossible *)
      ...
      | unifyExact (G, hasApproxType (tm, A), Vhs) =
	let
	  val ((tm', B, L), ok) = unifyExact (G, tm, Vhs)
	in
	  ((hasApproxType (tm', A), B, L), ok)
	end
    ...
    fun occElim (constant (H, r), os, rs, i) =
      ...
      | occElim (fvar (name, isP, r), os, rs, i) =
        let
          val r' = List.foldr Paths.join r rs
        in
          (Paths.root (r', Paths.leaf r, 0, i, os), r')
      ...
      | occElim (hasApproxType (tm, A), os, rs, i) = occElim (tm, os, rs, i)
    ...
    and occIntro (arrow (tm1, tm2)) =
      ...
      | occIntro (pi (dec (name, tm1, _, r), tm2)) =
        let
          val (oc1, r1) = occIntro tm1
          val (oc2, r2) = occIntro tm2
      ...
      | occIntro (lam (dec (name, tm1, _, r), tm2)) =
        let
          val (oc1, r1) = occIntro tm1
          val (oc2, r2) = occIntro tm2
          val r' = Paths.join (r, r2)
      ...
      | occIntro (hasApproxType (tm, A)) = occIntro tm
    ...
    fun inferExactJob (G, jnothing) = JNothing
      ...
      | inferExactJob (G, jscopeVars _) = raise Domain (* should be jscopeVars2 here! *)
      | inferExactJob (G, jscopeVars2 (tables, j)) =
	let
	  val oldApxTables = saveApxTables()
	  val oldExactTables = saveExactTables () (* also saves fvarCtx *)
	  val _ = restoreApxTables tables
	  val _ = setfvarCtx G (* New FVars now make sense in G *) 
	  val j' = inferExactJob (G, j)
	  val _ = restoreExactTables oldExactTables
	  val _ = restoreApxTables oldApxTables (* also restors fvarCtx *)
	in
	  j'
	end

      | inferExactJob (G, jpopctx (i, j)) =
	  JPopCtx (i, inferExactJob (popCtx(i, G), j))
	  ...
	  val V' = Apx.apxToClass (G, V, L, false)
		...
		val r = termRegion tm1
          ...
          val (tm1', B1) = checkExact (G, tm1, (V', id),
                                       "Delphin:  Ascription did not hold\n"
                                       ^ "(Index object(s) did not match)")
          val U1 = toIntro (B1, (V', id))
          val (oc1, r1) = occIntro tm1'
        in
          JOf ((U1, oc1), (V', oc1), Type)
        end

      | inferExactJob (G, jtypeEqualApx (tm1, U)) =
	  (* The inferApx phase made sure that U fits... *)
	  inferExactJob (G, jclass tm1)
