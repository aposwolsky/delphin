	    ...
	    if eqHead (Ha, Ha')
		 ...
		 then (* #succeeds = 1 *)
		   ((if (CSManager.trail (* trail to undo EVar instantiations *)
			(fn () => rSolve (ps', (r, I.comp(s, I.Shift(k))), dp,(I.BVar(I.Fixed k), Ha),
					  (fn S => (T.signal (G, T.SucceedGoal (tag, (I.BVar(I.Fixed k), Ha), I.EClo ps'));
						    raise SucceedOnce S)))))
		      then (* deep backtracking *)
			(T.signal (G, T.RetryGoal (tag, (I.BVar(I.Fixed k), Ha), I.EClo ps'));
			 ())
		    else (* shallow backtracking *)
		      ();
		      matchDProg (dPool', k+1))
		    handle SucceedOnce S => (T.signal (G, T.CommitGoal (tag, (I.BVar(I.Fixed k), Ha), I.EClo ps'));
					     sc (I.Root(I.BVar(I.Fixed k), S))))
		   
	       else (* #succeeds >= 1 -- allows backtracking *)
		 (if
		    CSManager.trail (fn () =>
				     rSolve (ps', (r, I.comp(s, I.Shift(k))),
					     dp, (I.BVar(I.Fixed k), Ha),
					     (fn S => (T.SucceedGoal (tag, (I.BVar(I.Fixed k), Ha), I.EClo ps');
						       sc (I.Root(I.BVar(I.Fixed k), S))))))
		    then (* deep backtracking *)
		      (T.signal (G, T.RetryGoal (tag, (I.BVar(I.Fixed k), Ha), I.EClo ps'));
		       ())
