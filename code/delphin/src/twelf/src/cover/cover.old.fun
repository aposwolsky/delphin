...
struct
  ...
  local
    ...
    fun weaken (I.Null, a) = I.id
      | weaken (I.Decl (G', D as I.Dec (name, V)), SOME a) =
	let
	  val w' = weaken (G', SOME a)
	  val Fopt = I.targetFamOpt V
	in
	  case Fopt
	    of NONE => I.dot1 w'
             | SOME F => 
	             if Subordinate.belowEq (F, a) then I.dot1 w'
		     else I.comp (w', I.shift)
	end
      | weaken (I.Decl (G', D as I.Dec (name, V)), NONE) =
	let
	  val w' = weaken (G', NONE)
	in
	  I.dot1 w'
    ...
    *)
    fun createEVar (G, V) =
        let (* G |- V : L *)
	  val w = weaken (G, I.targetFamOpt V)    (* G  |- w  : G'    *)
    ...
    *)
	...
	in
      ...
      | abbrevCGoal' (G, I.Uni I.Type, ci) = I.Uni I.Type
    ...
    *)
    ...
    fun buildSpine 0 = I.Nil
      | buildSpine n = (* n > 0 *)
        ...
        I.App (I.Root (I.BVar(I.Fixed n), I.Nil), buildSpine (n-1))
    ...
    *)
    ...
    and matchExpW (G, d, Us1 as (I.Root (H1, S1), s1), Us2 as (I.Root (H2, S2), s2), cands) =
      ...
      | matchExpW (G, d, (I.Uni I.Type, _), (I.Uni I.Type, _), cands) = cands
    ...
    *)
	...
	let val cands1 =
	   ...
	   | (I.Exp (U1), I.Idx (n2)) =>
		 matchExp (G, d, (U1, I.id), (I.Root (I.BVar (I.Fixed n2), I.Nil), I.id), cands)
           | (I.Idx (n1), I.Exp (U2)) =>
		 matchExp (G, d, (I.Root (I.BVar (I.Fixed n1), I.Nil), I.id), (U2, I.id), cands))
	in
    ...
    *)
	...
	(* Sat Dec 22 23:53:44 2001 -fp !!! *)
      ...
      | matchTopW (G, d, (I.Uni I.Type, _), (I.Uni I.Type, _), ci , cands) = cands
    ...
    *)
	...
	in
      ...
      | matchClause (G, ps', qs as (I.Uni I.Type, s), ci) =
          checkConstraints (G, qs, resolveCands (matchTop (G, 0, ps', qs, ci, Eqns (nil))))
    ...
    *)
    fun match (G, V as I.Root (I.Const (a), S), 0, ci, Input(ccs)) =
      ...
      | match (G, V as I.Uni I.Type, 0, ci, Input(ccs)) =
          matchCtx' (G, I.id, G, V, 1, ci,
		     matchSig (G, (V, I.id), ccs, ci, CandList (nil)))
    ...
    *)
	...
	in
      ...
      | createEVarSpineW (G, Vs as (I.Uni I.Type, _)) = (I.Nil, Vs)
    ...
    *)
    ...
    and occursInHead (k, I.BVar (I.Fixed k')) = (k = k')
    ...
    *)
    fun occursInMatchPos (k, I.Pi (DP, V), ci) =
      ...
      | occursInMatchPos (k, I.Uni I.Type, ci) = false
    ...
    *)
    ...
    fun targetBelowEq (SOME a, I.EVar (ref(NONE), GY, VY, ref nil)) =
         (case I.targetFamOpt VY
            of NONE => true (* this case may not be needed -- ABP *)
             | SOME F => Subordinate.belowEq(a, F))

      | targetBelowEq (NONE, I.EVar (ref(NONE), GY, VY, ref nil)) =
          true

      | targetBelowEq (aOpt, I.EVar (ref(NONE), GY, VY, ref (_::_))) =
    ...
    *)
	...
	I.Root (a, createCoverSpine (G, (S, s), (I.constType (cid), I.id), ms))
      ...
      | createCoverGoalW (G, (I.Uni I.Type, _), _, _) = I.Uni I.Type
  ...
  in
    ...
    *)
    fun substToSpine' (I.Shift(n), I.Null, T) = T
	  ...
	  (* Eta-expand *)
	let
	  val (Us,_) = Whnf.whnfEta ((I.Root (I.BVar(I.Fixed n), I.Nil), I.id), (V, I.id))
	in
