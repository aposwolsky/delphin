  ...
  *)
  local
    ...
    *)
    ...
    (* ABP:  Save state of varTable and indexTable *)
    val stateInformation : ((varEntry StringTree.Table) * (int StringTree.Table)) list ref = ref nil

    fun mark () = 
      let
	val vTable = StringTree.save varTable
	val iTable = StringTree.save indexTable
      in
	stateInformation := (vTable, iTable) :: (!stateInformation)
      end

    fun unwind () =
      let
	fun unwind' [] = raise Domain (* nothing marked to unwind to *)
	  | unwind' ((vTable, iTable)::rest) =
	          let
		    val _ = StringTree.restore (varTable, vTable)
		    val _ = StringTree.restore (indexTable, iTable)
		  in
		    rest
		  end
      in
	stateInformation := unwind' (!stateInformation)
      end
  in
    (* varReset () = ()
    ...
    fun bvarName (G, k) =
	   ...
	   of IntSyn.Dec(SOME(name), _) => name
	    ...
	    | IntSyn.NDec => (* "<_>" (* should be impossible *) *)
	                    ""
                            (* ABP (May, 2007):  It is possible when it converts
			     * a sub to a spine, and there are NDecs 
