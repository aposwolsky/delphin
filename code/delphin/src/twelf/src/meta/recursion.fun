...
struct
  ...
  local
     ...
     *)
    fun spine 0 = I.Nil 
      | spine n = I.App (I.Root (I.BVar (I.Fixed n), I.Nil),  spine (n-1))
...
(*      | checkLabels _ = NONE  (* more than one context block is introduced *) *)
	  ...
	  | set_parameter' ((I.Decl (G, D), I.Decl (B, S.Parameter _)), k, Ds) = 
		...
		CSManager.trail (fn () => 
			     if Unify.unifiable (G1, (V, I.id), (V', I.id))
			       andalso Unify.unifiable (G1, (X, I.id), (I.Root (I.BVar (I.Fixed k), I.Nil), I.id))
			       then sc Ds
			     else Ds)
	    in 
    ...
    *)
    ...
    and ltW (GB, k, (Us, Vs), ((I.Root (I.Const c, S'), s'), Vs'), sc, ac, Ds) = 
      ...
      | ltW (GB as (G, B), k, (Us, Vs), ((I.Root (I.BVar (I.Fixed n), S'), s'), Vs'), sc, ac, Ds) = 
	    ...
	    skolem ((du, de+1), (I.Decl (G, D2), I.Decl (B, T2)), I.comp (w, I.shift), F,
		    fn (s, de') => 
			 ...
			 val (s', V', F') = sc (s, de')
		       ...
		       in
			 (I.Dot (I.Exp (I.Root (I.BVar (I.Fixed (du + (de' - de))), spine du)), s'),
