  ...
  local
    ...
    *)
    fun abstractEVar (I.Decl (K', EV (r', _, _, d)), depth, X as I.EVar (r, _, _, _)) =
        if r = r' then (I.BVar (I.Fixed (depth+1)), d)
    ...
    *)
    fun abstractExpW (K, depth, (U as I.Uni (L), s)) = U
      ...
      | abstractExpW (K, depth, (I.Root (H as I.BVar (I.Fixed k), S), s)) = (* s = id *)
	  if k > depth then 
	    let 
	      val k' = lookupBV (K, k-depth)
	    in
	      I.Root (I.BVar (I.Fixed (k'+depth)), abstractSpine (K, depth, (S, s)))
	    end
	  else 
    ...
    *)
	...
	in
	  if Subordinate.belowEq (I.targetFam V, a) then 
	    (I.dot1 w', fn S => I.App (I.Root (I.BVar (I.Fixed i), I.Nil), S))
	...
	let
	  ...
	  val sc' = fn (w', k') => 
			...
			val s' = sc (w', k')
				        (* G0, GA, G[..] |- s' : G0, G, GF *)
		      in
			I.Dot (I.Exp (I.Root (I.BVar (I.Fixed (g+k'-k)), S''')), s')
