...
fun LFcollectExp (reg, (U, s), K, allowVars) =
  ...
  in
    ...
    (* abstractEVar (K, depth, X) = C'
       ...
       and  {{K}}, G |- C' : V
    *)
    fun abstractEVar (I.Decl (K', EV (_, I.EVar(r',GX,_,_))), depth, X as I.EVar (r, _, _, _)) =
    ...
    *)
    ...
    and abstractSub (K, depth, I.Shift (k), 0, S) = S
      | abstractSub (K, depth, I.Shift k, size, S) = abstractSub (K, depth, I.Dot (I.Idx (k+1), I.Shift (k+1)), size-1, S)
...
*)
    ...
    *)
    ...
    and collectDelFormula (D.Top _, K, allowVars) = K
      ...
      | collectDelFormula (D.FormulaString _, K, allowVars) = K
    ...
    fun addImplicitTypesForm F = 
               let
		 val K = collectDelFormula(F, I.Null, true)
		 val _ = checkConstraints K
		 val n = I.ctxLength K
