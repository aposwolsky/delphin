...
fun toHOAS_closed : <T:term> -> <maxInd z T> -> <exp> = toHOAS <z> (fn .) ;
params = .;
fun toHOAS_closed : <T:term> -> <maxInd z T> -> <exp> = toHOAS <z> (fn .) ;
(* BUG... this should also return a world error.. *)
fun foo = toHOAS <z> ;
fun bar = foo (fn .);
val foo2 = toHOAS <z> ;
val bar2 = foo2 (fn .);
...
val toDebruijn_closed : <exp> -> <T:term> * <maxInd z T> = toDebruijn <z> (fn .);
