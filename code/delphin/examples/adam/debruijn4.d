...
fun toDebruijn : <N:nat> -> (<E:exp#> -> <M: variable N> * <trans E (bvar' M)>) -> <E:exp> -> <T:term N> * <trans E T>
    = fn <N> W <app E1 E2> => 
       ...
       | <N> W <lam E> => 
             (case ({<x:exp>}{<d:trans x (bvar' (var_one : variable (s N)))>} toDebruijn <s N> ((extend W) \x \d) <E x>)
...
fun toHOAS : <N:nat> -> (<M:variable N> -> <E:exp> * <trans E (bvar' M)>) -> <T:term N> -> <E:exp> * <trans E T>
   = fn <N> W <app' T1 T2> =>
      ...
      | <N> W <lam' T> =>
	     (case ({<x:exp>}{<d:trans x (bvar' (var_one : variable (s N)))>} toHOAS <s N> 
                                   (fn <var_one> => (<x>, <d>)
				    | <var_s M> => let 
						     val (<E>, <D>) = W <M>
				                   in
...
type iso2Contra3 = <E:exp#> -> <N:nat> -> <trans E (bvar' (var_one : variable (s N))) #> -> <B: variable N> -> <trans E (bvar' B)> -> <empty> ;
...
fun extendContradiction : iso2Contra3
             -> {<x>}{<d: trans x (bvar' (var_one : variable (s N) ))>} iso2Contra3
...
fun iso2 : iso2Contra1
	 ...
	 | C1 C2 C3 W <bvar' var_one> <bvar' (var_s B2)> <E#> <D1#> <trans_var D2> => 
		    let  
		      val (<M'>, <L'>, <B2'>, <D2'>) = iso2Var <B2> <trans_var D2>
		      val <bot> = C3 <E> <M'> <s N> <L'> <var_one> <D1> <B2'> <D2'>
		    in
		      contra <bot>
		    end
	 | C1 C2 C3 W <bvar' (var_s B1)> <bvar' var_one>  <E#> <trans_var D1> <D2#> => 
		    let
		      val (<M'>, <L'>, <B1'>, <D1'>) = iso2Var <B1> <trans_var D1>
		      val <bot> = C3 <E> <_> <_> <L'> <var_one> <D2> <B1'> <D1'>
		    in
		      contra <bot>
		    end;
